...
function tcl_test_google_ads_api_connection() {
    ...
    if (!file_exists($vendor_path)) {
        return [
            'success' => false,
            'error' => 'Google Ads PHPライブラリがインストールされていません。Composerタブでインストールしてください。'
        ];
    }
    ...
    try {
        ...
        for ($version = 15; $version <= 17; $version++) {
            $class_name = "Google\\Ads\\GoogleAds\\Lib\\V{$version}\\GoogleAdsClientBuilder";
            if (class_exists($class_name)) {
                $available_versions[] = "v{$version}";
            }
        }
        ...
        if (empty($available_versions)) {
            return [
                'success' => false,
                'error' => 'Google Ads APIクライアントクラスが見つかりません。ライブラリの再インストールが必要です。'
            ];
        }
        
        // 最新の利用可能バージョンを使用
        $latest_version = max(array_map(function($v) { return intval(substr($v, 1)); }, $available_versions));
        $client_builder_class = "Google\\Ads\\GoogleAds\\Lib\\V{$latest_version}\\GoogleAdsClientBuilder";
        ...
        // Google Ads APIクライアントを初期化（テスト用）
        $googleAdsClient = (new $client_builder_class())
        ...
        return [
            'success' => true,
            'message' => "Google Ads API接続成功 (v{$latest_version}利用可能)",
...
/**
 * 簡易版Google Ads API設定チェック（重複を避けるため）
 */
...
/**
 * Google Ads API用の一時設定ファイルを作成（重複を避けるため）
 */
