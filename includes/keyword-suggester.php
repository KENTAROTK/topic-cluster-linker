...
private function analyze_content_for_keywords($post) {
    if (empty($this->api_key)) {
        return $this->get_fallback_keywords($post);  // ★ この行だけ変更 ★
    }
    ...
    try {
        ...
        tcl_log_message('キーワード提案エラー: ' . $e->getMessage());
        return $this->get_fallback_keywords($post);
    }
    // ★ try-catch を追加（ここまで） ★
}
...
private function call_keyword_analysis_api($content, $title) {
    ...
    // ★ デバッグログ追加（ここから） ★
    tcl_log_message('=== ChatGPT API呼び出し開始 ===');
    tcl_log_message('APIキー設定状況: ' . (!empty($this->api_key) ? '設定済み' : '未設定'));
    tcl_log_message('APIキー長さ: ' . strlen($this->api_key));
    tcl_log_message('分析対象タイトル: ' . $title);
    tcl_log_message('コンテンツ長さ: ' . strlen($content));
    // ★ デバッグログ追加（ここまで） ★
...
// ★ ここからログ追加 ★
tcl_log_message('=== ChatGPT APIレスポンス ===');

if (is_wp_error($response)) {
    tcl_log_message('WordPress HTTP エラー: ' . $response->get_error_message());
    throw new Exception('API通信エラー: ' . $response->get_error_message());
}

$response_code = wp_remote_retrieve_response_code($response);
tcl_log_message('HTTPレスポンスコード: ' . $response_code);

$body = wp_remote_retrieve_body($response);
$data = json_decode($body, true);

tcl_log_message('レスポンスボディ長さ: ' . strlen($body));
tcl_log_message('JSONデコード結果: ' . (is_array($data) ? 'OK' : 'NG'));

if (!empty($data['error'])) {
    tcl_log_message('OpenAI APIエラー: ' . print_r($data['error'], true));
    throw new Exception('OpenAI API エラー: ' . $data['error']['message']);
}

if (empty($data['choices'][0]['message']['content'])) {
    tcl_log_message('空のコンテンツレスポンス');
    tcl_log_message('レスポンス全体: ' . print_r($data, true));
    throw new Exception('空のレスポンス');
}

tcl_log_message('取得したコンテンツ: ' . substr($data['choices'][0]['message']['content'], 0, 200) . '...');
tcl_log_message('=== ChatGPT API呼び出し完了 ===');
// ★ ここまでログ追加 ★
...
return trim($data['choices'][0]['message']['content']);
     ...
     * フォールバック用キーワード提案
     */
    private function get_fallback_keywords($post) {
        $title = $post->post_title;
        $content = wp_strip_all_tags($post->post_content);
        $combined_text = $title . ' ' . $content;
        
        // よく使われる単語を抽出して業界を推測
        $keywords = $this->extract_keywords_from_content($combined_text);
        
        return [
            'keywords' => $keywords,
            'analysis' => '投稿内容から抽出されたキーワードです。ChatGPT APIキーを設定するとより精度の高い分析が可能になります。',
            'pillar_potential' => 'APIキー設定後により詳細な分析が可能です'
        ];
    }
     ...
     * コンテンツから動的にキーワードを抽出
     */
    private function extract_keywords_from_content($text) {
        // テキストを単語に分割
        $text = mb_convert_kana($text, 'as'); // 全角英数を半角に
        $words = preg_split('/[\s\p{P}\p{Z}]+/u', $text, -1, PREG_SPLIT_NO_EMPTY);
        
        // 除外する一般的な単語
        $stop_words = [
            'です', 'ます', 'する', 'ある', 'この', 'その', 'それ', 'これ', 'また', 'さらに',
            'について', 'により', 'ため', 'こと', 'もの', 'など', 'として', 'では',
        ...
        // 出現回数でソートして上位を取得
        arsort($word_count);
        $top_words = array_slice(array_keys($word_count), 0, 8);
        
        // 十分なキーワードが見つからない場合は汎用キーワードを追加
        if (count($top_words) < 5) {
            $generic_keywords = [
                'サービス', '方法', '対策', '解決', '改善', '効果',
                '基本', '応用', '実践', '活用', '導入', '運用'
            ];
            
            $top_words = array_merge($top_words, $generic_keywords);
            $top_words = array_slice(array_unique($top_words), 0, 8);
        }
        
return $top_words;
    }  // extract_keywords_from_content メソッドの終了

}  // TCL_KeywordSuggester クラスの終了
